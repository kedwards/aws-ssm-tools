...
@test "preserves variable default syntax" {
  printf '# Use default\necho ${MYVAR:-default_value}\n' > "$HOME/.config/aws-tools/commands/ssm/default-var"
  menu_select_one() {
    local result_var="$3"; shift 3
    printf -v "$result_var" '%s' "$1"
    return 0
...
@test "preserves quoted strings" {
  printf '# Mixed quotes\necho "User: $USER" '"'"'Literal: $USER'"'"'\n' > "$HOME/.config/aws-tools/commands/ssm/mixed-quotes"
  menu_select_one() {
    local result_var="$3"; shift 3
    printf -v "$result_var" '%s' "$1"
    return 0
...
@test "preserves here-string syntax" {
  printf '# Here string\ngrep pattern <<< "test string"\n' > "$HOME/.config/aws-tools/commands/ssm/here-string"
  menu_select_one() {
    local result_var="$3"; shift 3
    printf -v "$result_var" '%s' "$1"
    return 0
...
@test "preserves array syntax" {
  printf '# Array variable\necho ${FILES[0]}\n' > "$HOME/.config/aws-tools/commands/ssm/array-var"
  menu_select_one() {
    local result_var="$3"; shift 3
    printf -v "$result_var" '%s' "$1"
    return 0
...
@test "preserves parameter length syntax" {
  printf '# Parameter length\necho ${#USER}\n' > "$HOME/.config/aws-tools/commands/ssm/param-length"
  menu_select_one() {
    local result_var="$3"; shift 3
    printf -v "$result_var" '%s' "$1"
    return 0
...
@test "preserves string substitution syntax" {
  printf '# String substitution\necho ${USER/test/prod}\n' > "$HOME/.config/aws-tools/commands/ssm/string-sub"
  menu_select_one() {
    local result_var="$3"; shift 3
    printf -v "$result_var" '%s' "$1"
    return 0
...
@test "handles command file with extra blank lines between body lines" {
  printf '# Spaced out\necho line1\n\necho line2\n' > "$HOME/.config/aws-tools/commands/ssm/spaced"
  menu_select_one() {
    local result_var="$3"; shift 3
    printf -v "$result_var" '%s' "$1"
    return 0
