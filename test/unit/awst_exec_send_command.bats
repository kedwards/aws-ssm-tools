...
@test "awst_exec creates temp file for send-command" {
...
@test "awst_exec creates JSON with command and timeout" {
  local aws_log="/tmp/bats-aws-$$"
  aws() {
    echo "$*" >> "$aws_log"
    if [[ "$1" == "ssm" && "$2" == "send-command" ]]; then
...
@test "awst_exec calls aws ssm send-command with instance IDs" {
  local aws_log="/tmp/bats-aws-$$"
  aws() {
    echo "$*" >> "$aws_log"
    if [[ "$1" == "ssm" && "$2" == "send-command" ]]; then
...
@test "awst_exec uses AWS-RunShellScript document" {
  local aws_log="/tmp/bats-aws-$$"
  aws() {
    echo "$*" >> "$aws_log"
    if [[ "$1" == "ssm" && "$2" == "send-command" ]]; then
...
@test "awst_exec passes multiple instance IDs to send-command" {
  local aws_log="/tmp/bats-aws-$$"
  aws() {
    echo "$*" >> "$aws_log"
    if [[ "$1" == "ssm" && "$2" == "send-command" ]]; then
...
@test "awst_exec extracts command ID from send-command output" {
  aws() {
    if [[ "$1" == "ssm" && "$2" == "send-command" ]]; then
...
@test "awst_exec fails if send-command fails" {
  aws() {
    if [[ "$1" == "ssm" && "$2" == "send-command" ]]; then
      return 1
    fi
...
@test "awst_exec cleans up temp file on success" {
  local tmpfile_created=""
  local tmpfile_removed=""
  mktemp() {
    tmpfile_created="/tmp/awst-test-$$"
...
@test "awst_exec cleans up temp file on failure" {
  local tmpfile="/tmp/awst-test-$$"
  local removed_file=""
  mktemp() {
